...
function validarFormatoEmail(req, res, next) {
  // Validar el formato del email con express-validator
  body('email').isEmail().withMessage('Ingrese un email correcto.')(req, res, function () {
...
function eliminarCurso(req, res, next){
  const alumno = req.alumno;
  const codigoCurso = req.codigoCurso;
  const index = alumno.cursos.findIndex(curso => curso.codigo === codigoCurso);
...
// Alumno por ID
app.get("/alumno/:id", setearIdAlumno, validarIdEnRuta, obtenerAlumnoPorId, okMiddlewareGet, badRequest)
// Alumno nuevo
app.post("/alumno", setearDatosAlumno, validarCamposRequeridos, validarIdEnBody, validarFormatoEmail, validarEmailRepetido, validarIDrepetido, okMiddlewarePost, badRequest)
// Borrar alumno
app.delete("/alumno/:id", setearIdAlumno, validarIdEnRuta, obtenerAlumnoPorId, borrarAlumno, okMiddlewareDelete, badRequest)
// Agrergar un nuevo curso a un alumno
app.put("/alumno/:id/curso", setearIdAlumno, validarIdEnRuta, obtenerAlumnoPorId, agregarCurso, okMiddlewarePut, badRequest)
// Quitar curso de un alumno
app.patch("/alumno/:id/curso/:codigo", setearIdAlumno, setearCodigoCurso,validarIdEnRuta, obtenerAlumnoPorId, validarCurso, eliminarCurso, okMiddlewarePatch, badRequest)
